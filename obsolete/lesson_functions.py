...
def single_img_scaled_features(img, color_space='RGB', spatial_size=(32, 32),
    ...
    # Convert to float32 for more accurate color conversions and scale to 0..1
...
def single_img_features(img, color_space='RGB', spatial_size=(32, 32),
    ...
    # Here is quick fix so we can use existing code, OpenCV uses BGR as
    # default and original udacity code uses RGB color space
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)


    # TODO: fix this and below if-elif structure
    # 2) Apply color conversion if other than 'RGB'
    if color_space != 'RGB':
        if color_space == 'HSV':
...
def extract_scaled_features(imgs, color_space='RGB', spatial_size=(32, 32),
                     hist_bins=32, orient=9,
                     pix_per_cell=8, cell_per_block=2, hog_channel=0,
                     spatial_feat=True, hist_feat=True, hog_feat=True):
    ...
    for img in imgs:
        f = single_img_scaled_features(img,color_space=color_space,
...
def extract_features2(imgs, color_space='RGB', spatial_size=(32, 32),
    ...
    # Iterate through the list of images
    for img in imgs:
        feat = single_img_features(img,color_space=color_space,
                                   spatial_size=spatial_size,
                                   hist_bins=hist_bins,
                                   orient=orient,
                                   pix_per_cell=pix_per_cell,
